...
#This creates an example text, which is the first line from a book I am reading
...
# essentially lists raw entities by loop
if doc.ents:
    for ent in doc.ents: # loops through each entity
        st.write(f"**{ent.text}** - {ent.label_}") # dislplays text and its corresponding label
else:
    st.write("0 entities found.")

st.header("Processed Text")

#Should get custom colors, not sure if working
...
# this shows the current custom patterns in the session state
